...
export default function Dashboard({ auth }) {
    ...
    const noPicturesFound = <>
        <div className={sty.empty_picture_container}>
            <IconPhotoOff style={{ opacity: 0.9 }} color='var(--mantine-primary-color-filled-hover)' size={'30%'} stroke={1.25} />
            <Text className={sty.main}>NO PHOTOS FOUND</Text>
            <Text className={sty.desc}>Looks like you’re so stunning that even the internet couldn’t handle your photos</Text>
        </div>
    </>;
        ...
        <AuthLayout auth={auth}>
            <div className={sty.container}>
                {(!images) ? // getting a list of pictures to load
                    ...
                    :
                    <>
                    {images.length === 0 ? noPicturesFound : // Displaying no pictures found message
                        <> 
                        {images.map((img, i) => // Loading actual pictures
                            <LazyLoadImage
                                key={i}
